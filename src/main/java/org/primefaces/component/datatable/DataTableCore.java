...
@PFComponent(tagName = "dataTable",
             ...
             widget = true,
             rtl = true)
public abstract class DataTableCore extends UIData implements IDataTable, javax.faces.component.behavior.ClientBehaviorHolder, org.primefaces.component.api.PrimeClientBehaviorHolder, org.primefaces.component.api.Pageable {
	...
	public enum PropertyKeys {
		...
		scrollHeight,
		@PFProperty(description = "Width for scrollable data")
		scrollWidth,
		@PFProperty(description = "Enables data selection, valid values are \"single\" and \"multiple\"")
		selectionMode,
		@PFProperty(description = "Reference to the selection data", type = Object.class)
		selection,
		...
		styleClass,
		@PFProperty(description = "Enables live scrolling. Default is false", defaultValue = "false", type = Boolean.class)
		liveScroll,
		@PFProperty(description = "Style class for each row")
		rowStyleClass,
		@PFProperty(description = "Client side callback to execute before row expansion")
		onExpandStart,
		@PFProperty(description = "Decides whether datatable columns are resizable or not", defaultValue = "false", type = Boolean.class)
		resizableColumns,
		@PFProperty(description = "Property to be used for default sorting", type = Object.class)
		sortBy,
		@PFProperty(description = "Sets sorting order. Default is \"ascending\"", defaultValue = "ascending")
		sortOrder,
		...
		scrollRows,
		@PFProperty(description = "Unique identifier of row data", type = Object.class)
		rowKey,
		@PFProperty(description = "Client side event to invoke datatable filtering for input fields. Default is keyup")
		filterEvent,
		...
		filterDelay,
		@PFProperty(description = "Inline style of the table element")
		tableStyle,
		@PFProperty(description = "Style class of the table element")
		tableStyleClass,
		@PFProperty(description = "Decides whether datatable columns can be reordered using dragdrop", defaultValue = "false", type = Boolean.class)
		draggableColumns,
		@PFProperty(description = "Controls incell editing", defaultValue = "false", type = Boolean.class)
		editable,
		@PFProperty(description = "List to keep the filtered data if filtering is enabled", type = java.util.List.class)
		filteredValue,
		@PFProperty(description = "Defines sorting mode, valid values are \"single\" (default) and \"multiple\"", defaultValue = "single")
		sortMode,
		...
		cellSeparator,
		@PFProperty(description = "Summary attribute for WCAG")
		summary,
		@PFProperty(description = "Number of rows to freeze starting from the beginning", defaultValue = "0", type = Integer.class)
		frozenRows,
		@PFProperty(description = "Columns are resized live in this mode without using a resize helper", defaultValue = "false", type = Boolean.class)
		liveResize,
		@PFProperty(description = "Sticky header stays in window viewport during scrolling", defaultValue = "false", type = Boolean.class)
		stickyHeader,
		@PFProperty(description = "Defines if row should be rendered as expanded by default", defaultValue = "false", type = Boolean.class)
		expandedRow,
		@PFProperty(description = "Disables row selection when true", defaultValue = "false", type = Boolean.class)
		disabledSelection,
		@PFProperty(description = "Defines row select mode for checkbox based selection, valid values are new (default), add and checkbox", defaultValue = "new")
		rowSelectMode,
		@PFProperty(description = "Defines row expand mode, valid values are \"single\" and \"multiple\" (default)", defaultValue = "multiple")
		rowExpandMode,
		@PFProperty(description = "Locale to be used in features such as filtering and sorting, defaults to view locale", type = Object.class)
		dataLocale,
		...
		nativeElements,
		@PFProperty(description = "Number of columns to freeze, default is 0", defaultValue = "0", type = Integer.class)
		frozenColumns,
		...
		caseSensitiveSort,
		@PFProperty(description = "Ignores processing of children during lifecycle, improves performance if table only has output components, default is false", defaultValue = "false", type = Boolean.class)
		skipChildren,
		...
		@PFProperty(description = "Name of the field to pass lazy load method for sorting. If not specified, sortBy expression is used to extract the name")
		sortField,
		@PFProperty(description = "Defines when the datatable is initiated at client side, valid values are \"load\" (default) and \"immediate\"", defaultValue = "load")
		initMode,
		@PFProperty(description = "Defines where the null values are placed in ascending sort order. Default value is \"1\" meaning null values are placed at the end in ascending mode and\n at beginning in descending mode. Set to \"-1\" for the opposite behavior", defaultValue = "1", type = Integer.class)
		nullSortOrder,
		@PFProperty(description = "Position of the element in the tabbing order", defaultValue = "0")
		tabindex,
		@PFProperty(description = "Reflow mode is a responsive mode to display columns as stacked depending on screen size", defaultValue = "false", type = Boolean.class)
		reflow,
		@PFProperty(description = "Percentage height of the buffer between the bottom of the page and the scroll position to initiate\n the load for the new chunk. Value is defined in integer and default is 0", defaultValue = "0", type = Integer.class)
		liveScrollBuffer,
		@PFProperty(description = "Adds hover effect to rows, default is false. Hover is always on when selection is enabled", defaultValue = "false", type = Boolean.class)
		rowHover,
		@PFProperty(description = "Defines the resize behavior, valid values are \"fit\" (default) and expand", defaultValue = "fit")
		resizeMode,
		@PFProperty(description = "Label to read by screen readers on checkbox selection")
		ariaRowLabel,
		...
		saveOnCellBlur,
		@PFProperty(description = "Caches the next page asynchronously, default is false", defaultValue = "false", type = Boolean.class)
		clientCache,
		@PFProperty(description = "Whether to keep table state across views, defaults to false", defaultValue = "false", type = Boolean.class)
		multiViewState,
		@PFProperty(description = "List of FilterState objects to filter table by default", type = java.util.List.class)
		filterBy,
		@PFProperty(description = "Value of the global filter to use when filtering by default")
		globalFilter,
		@PFProperty(description = "Defines the cell edit behavior, valid values are \"eager\" (default) and \"lazy\"", defaultValue = "eager")
		cellEditMode,
		@PFProperty(description = "Makes row groups toggleable, default is false", defaultValue = "false", type = Boolean.class)
		expandableRowGroups,
		@PFProperty(description = "Loads data on demand as the scrollbar gets close to the bottom. Default is false", defaultValue = "false", type = Boolean.class)
		virtualScroll,
		@PFProperty(description = "Defines the element used to reorder rows using dragdrop. Default selector is \"td,span:not(.ui-c)\"")
		rowDragSelector,
		@PFProperty(description = "Method expression to execute after dragging row", type = javax.el.MethodExpression.class)
		draggableRowsFunction,
		@PFProperty(description = "Client side callback to execute after clicking row")
		onRowClick,
		@PFProperty(description = "Defines a client side event to open cell on editable table", defaultValue = "click")
		editInitEvent,
		@PFProperty(description = "Client side check if rowclick triggered row click event not a clickable element in row content")
		rowSelector,;
	}
